...
public class CreateEventRepository {
	...
	// Fill Form
		...
		public static WebElement content(WebDriver driver) {
			WebElement iframeWeb =  driver.findElement(By.xpath("//div[@id='cke_3_contents']/child::iframe"));
			WebDriverWait block = new WebDriverWait(driver, 30);
			block.until(ExpectedConditions.visibilityOf(iframeWeb));
			driver.switchTo().frame(iframeWeb);
		...
		public static WebElement StartDate(WebDriver driver) {
			...
			element = driver.findElement(By.xpath("(//span[contains(text(),'StartDate')]/following-sibling::span/input[@placeholder='mm/dd/yyyy'])[3]"));
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("(//span[contains(text(),'StartDate')]/following-sibling::span/input[@placeholder='mm/dd/yyyy'])[3]")));
			/*JavascriptExecutor jse = (JavascriptExecutor) driver;
...
*/		}
		public static WebElement StartTime(WebDriver driver) {
			wait=new WebDriverWait(driver,40);
			element = driver.findElement(By.xpath("(//span[contains(text(),'StartDate')]/following-sibling::span[@class='lfr-input-time timepicker-input']/input[@placeholder='h:mm am/pm'])[3]"));
			wait.until(ExpectedConditions.visibilityOf(element));
			/*JavascriptExecutor jse = (JavascriptExecutor) driver;
			jse.executeScript("arguments[0].click();", element);
			ArrayList<WebElement> allspans=(ArrayList<WebElement>) element.findElements(By.tagName("input"));
		...
		public static WebElement EndDate(WebDriver driver) {
			wait=new WebDriverWait(driver,40);
			element = driver.findElement(By.xpath("(//span[contains(text(),'EndDate')]/following-sibling::span/input[@placeholder='mm/dd/yyyy'])[3]"));
...
*/		}
		public static WebElement EndTime(WebDriver driver) {
			wait=new WebDriverWait(driver,40);
			element = driver.findElement(By.xpath("(//span[contains(text(),'EndDate')]/following-sibling::span[@class='lfr-input-time timepicker-input']/input[@placeholder='h:mm am/pm'])[3]"));
			wait.until(ExpectedConditions.visibilityOf(element));
			/*JavascriptExecutor jse = (JavascriptExecutor) driver;
			jse.executeScript("arguments[0].click();", element);
			ArrayList<WebElement> allspans=(ArrayList<WebElement>) element.findElements(By.tagName("input"));
